...
void host_server(ENetHost* server)
{
	...
	while (enet_host_service(server, &event, 2) > 0)
	{
		...
		case ENET_EVENT_TYPE_RECEIVE:
			printf("A packet of length %zu containing %s was received from %s "
			       "on channel %u.\n",
...
/**
 * @brief Demonstrate the common usage for ENet library in C/C++.
 *
 * @return int
 */
int main()
...
#define MAX_CLIENTS 5
	...
	do
	{
		...
		for (i = 0; i < MAX_CLIENTS; ++i)
		{
			int e = enet_host_service(clients[i].host, &event, 0);

			if (e > 0)
			{
				switch (event.type)
				{
				case ENET_EVENT_TYPE_RECEIVE: printf("recv from client");
				default: break;
				}
			}
		}
